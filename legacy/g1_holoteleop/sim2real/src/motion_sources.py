...
class VRMotionSource(MotionSourceBase):
    def __init__(self, policy: "TrackingPolicyRaw", policy_cfg: DictToClass):
        ...
        if self.vr_transport == "zmq":
            ...
            try:
                ...
                print(
                    "[VRMotionSource] Connected "
                    f"transport=udp_stream, stream<-udp://{self.vr_udp_stream_bind_addr}:{self.vr_udp_stream_port}, "
                ...
                print(f"[VRMotionSource] Failed to create UDP stream socket: {e}")
    ...
    def _apply_control_payload(self, latest_payload: Optional[dict], latest_buttons: dict, drained: int) -> None:
        ...
        set_hand_fn = getattr(self.policy.controller, "set_dex3_from_controller_buttons", None)
        if callable(set_hand_fn):
