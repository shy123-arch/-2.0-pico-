...
class RuntimeReferenceUDPClient:
    """UDP client for streaming the current runtime reference skeleton."""
    def __init__(self, host: str = "127.0.0.1", port: int = 28564):
        self._host = host
        self._port = port
        self._sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
class RuntimeReferenceUDPServer(threading.Thread):
    """Non-blocking UDP server that stores the latest runtime reference skeleton."""
    def __init__(self, host: str = "127.0.0.1", port: int = 28564):
        super().__init__(daemon=True)
        self._host = host
        self._port = port
        ...
        print(f"[RuntimeReferenceUDPServer] Listening on udp://{host}:{port}")
