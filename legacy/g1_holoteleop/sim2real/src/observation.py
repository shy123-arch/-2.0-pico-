...
class TargetRootZObs:
    def __init__(self, policy):
        self.policy = policy
        self.future_steps = _policy_future_steps(policy)
        self.offset = float(getattr(getattr(policy, "config", None), "target_root_z_offset", 0.035))
    ...
    def compute(self) -> np.ndarray:
        ...
        return (root_pos_w[:, 2] + self.offset).astype(np.float32)
...
class TargetPosBObs:
    ...
    def _robot_root_pos_w(self) -> np.ndarray:
        for attr in ("root_pos_w", "root_pos", "base_pos_w", "base_pos"):
            if hasattr(self.ctrl, attr):
                value = getattr(self.ctrl, attr)
                if value is not None:
                    return np.asarray(value, dtype=np.float32).reshape(3)

        base = self.policy.ref_idx
        return self.policy.ref_root_pos[base].astype(np.float32, copy=True)
    ...
    def compute(self) -> np.ndarray:
        ...
        T = self.policy.ref_len
        fut_idx = _clamp_indices(base + self.future_steps, T)
        target_pos_w = self.policy.ref_root_pos[fut_idx].copy()

        robot_root_pos_w = self._robot_root_pos_w()
        robot_root_quat_w = (
            np.asarray(self.ctrl.root_quat_w, dtype=np.float32).reshape(4)
            if hasattr(self.ctrl, "root_quat_w") and self.ctrl.root_quat_w is not None
            else self.ctrl.quat.copy()
        )
        pos_diff_w = target_pos_w - robot_root_pos_w.reshape(1, 3)
        pos_diff_b = _quat_apply_inv(robot_root_quat_w, pos_diff_w)
        return pos_diff_b.reshape(-1).astype(np.float32)
