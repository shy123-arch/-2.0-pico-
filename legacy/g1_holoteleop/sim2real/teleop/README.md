...
## PICO and GMR recording

Raw PICO/XRobot frames and the synchronized online GMR result can be recorded together:
...
If `GMR_RECORD_OUTPUT_DIR` is not set, the online GMR files are written next to the raw directory under `gmr_records`.

Controls:

- right controller B: start recording
- left controller Y: stop and save
- keyboard `r`: start recording
- keyboard `p`: stop and save
- keyboard `q`: save and quit

The saved files use matching session and segment ids:

```text
/home/velix/project/HoloTeleop/data/pico_records/pico_raw_YYYYMMDD_HHMMSS_seg000000.npz
/home/velix/project/HoloTeleop/data/gmr_records/gmr_mt_YYYYMMDD_HHMMSS_seg000000.npz
```

The raw files contain:

- `raw_body_poses`: `(T, 24, 7)` PICO body poses, `[x, y, z, qx, qy, qz, qw]`
- `body_joint_names`
- `raw_headset_pose`
- `raw_left_controller_pose`
- `raw_right_controller_pose`
- `raw_controller_buttons`
- `raw_controller_values`
- `raw_recv_ns`, `raw_seq`, and source timestamps

The GMR files contain online retargeted MT-style data:
...
- `joint_names`
- `fps`
- `aligned_seq`, `aligned_raw_indices`, `aligned_gmr_indices`

Use the GMR file for quick visualization and quality checks. For final training data, keep the raw files as the source of truth and convert them offline.

Replay the latest matching raw/GMR pair in MuJoCo:

```bash
cd /home/velix/project/HoloTeleop/sim2real
source $HOME/.local/bin/env
uv run teleop/replay_pico_gmr_mujoco.py --mode both --loop
```

Replay only one side:
...
`both` mode aligns coordinates by translating the first raw PICO `Pelvis` position onto the first GMR robot root position, so XY drift can be compared in the same MuJoCo world frame.
