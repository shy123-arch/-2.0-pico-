...
from common.runtime_reference import RuntimeReferencePublisher
...
class Policy:
    ...
    def _debug_event(self, key: str, msg: str, interval_s: float = 1.0):
        now = time.monotonic()
    ...
    def update_obs(self):
        ...
        summary = self._nonfinite_summary("obs", obs)
        if summary is not None:
            self._debug_event(
                "obs_nonfinite",
                f"[Policy:{self.name}][Error] observation contains nan/inf | {summary}",
            )
        if self.policy_input is None:
            ...
            self.policy_input["policy"][0, :] = obs
    ...
    def compute_action(self) -> np.ndarray:
        ...
        raw_action_isaac = out["action"].copy()[0].astype(np.float32)
        summary = self._nonfinite_summary("raw_action", raw_action_isaac)
        if summary is not None:
            self._debug_event(
                "raw_action_nonfinite",
                f"[Policy:{self.name}][Error] ONNX raw action contains nan/inf | {summary}",
            )
        action_isaac = raw_action_isaac.clip(-self.action_clip, self.action_clip)
        self.last_action[:] = action_isaac
        self.applied_action_isaac[:] = action_isaac * self.action_scale_isaac

        if self._action_debug_count < 5:
            order = np.argsort(-np.abs(self.applied_action_isaac))[:8]
            top = ", ".join(
                f"{self.action_joint_names[int(i)]}:raw={float(raw_action_isaac[int(i)]):+.3f},"
                f"scaled={float(self.applied_action_isaac[int(i)]):+.3f}"
                for i in order
            )
            print(
                f"[Policy:{self.name}][ActionDiag] "
                f"count={self._action_debug_count} "
                f"raw_abs_max={float(np.max(np.abs(raw_action_isaac))):.3f} "
                f"scaled_abs_max={float(np.max(np.abs(self.applied_action_isaac))):.3f} "
                f"top=[{top}]"
            )
...
class TrackingPolicyRaw(Policy):
    ...
    def __init__(self, name: str, policy_cfg: DictToClass, controller):
        ...
        self.runtime_reference_enable = bool(getattr(policy_cfg, "runtime_reference_enable", True))
        self.runtime_reference_host = str(getattr(policy_cfg, "runtime_reference_host", "127.0.0.1"))
        self.runtime_reference_port = int(getattr(policy_cfg, "runtime_reference_port", 28564))
        self.uses_root_xy_obs = False
        ...
        self._runtime_reference_publisher: Optional[RuntimeReferencePublisher] = None
        if self.runtime_reference_enable:
            try:
                self._runtime_reference_publisher = RuntimeReferencePublisher(
                    urdf_path=str(ASSETS_DIR / "g1" / "g1_29dof_rev_1_0_policy_fk.urdf"),
                    joint_names_state=list(self.obs_joint_names),
                    host=self.runtime_reference_host,
                    port=self.runtime_reference_port,
                )
            except Exception as e:
                print(f"[TrackingPolicyRaw] Failed to start runtime reference publisher: {e}")
    ...
    def deactivate(self):
        ...
        if self._runtime_reference_publisher is not None:
            self._runtime_reference_publisher.close()
        self.ref_root_pos = None
        self.ref_root_quat = None
        self.ref_joint_pos = None
        super().deactivate()

    def _publish_runtime_reference(self):
        if self._runtime_reference_publisher is None:
            return
        if self.ref_joint_pos is None or self.ref_root_pos is None or self.ref_root_quat is None or self.ref_len <= 0:
            return
        try:
            root_pos = self.ref_root_pos[self.ref_idx].astype(np.float32, copy=True)
            odom_origin_xy = getattr(self.controller, "_odom_origin_xy", None)
            if self.uses_root_xy_obs and odom_origin_xy is not None:
                root_pos[:2] += np.asarray(odom_origin_xy, dtype=np.float32).reshape(2)
            self._runtime_reference_publisher.publish(
                motion_name=self.current_name,
                joint_pos_state=self.ref_joint_pos[self.ref_idx],
                root_pos=root_pos,
                root_quat_wxyz=self.ref_root_quat[self.ref_idx],
            )
        except Exception:
            pass
    ...
    def _build_obs_modules(self):
        ...
        if self.module is not None:
            input_shape = self.module.ort_session.get_inputs()[0].shape
            if len(input_shape) > 0 and isinstance(input_shape[-1], int):
        ...
        self.uses_root_xy_obs = expected_obs_dim == 1623
        if self.uses_root_xy_obs:
            self.obs_modules.append(TargetPosBObs(self.controller, self))

        self.obs_modules.extend([
    ...
    def update_obs(self):
        ...
        self._publish_runtime_reference()
        super().update_obs()
    ...
    def read_robot_state(self) -> Dict[str, np.ndarray]:
        q_policy = self.controller.qj_isaac.copy().astype(np.float32)
        root_pos_w = getattr(self.controller, "root_pos_w", None)
        root_quat_w = getattr(self.controller, "root_quat_w", None)
        if root_pos_w is not None:
            root_pos = np.asarray(root_pos_w, dtype=np.float32).reshape(3).copy()
        elif self.ref_root_pos is not None:
            root_pos = self.ref_root_pos[self.ref_idx].astype(np.float32, copy=True)
        else:
            root_pos = np.array([0.0, 0.0, 0.78], dtype=np.float32)

        if root_quat_w is not None:
            root_quat = np.asarray(root_quat_w, dtype=np.float32).reshape(4).copy()
        else:
            root_quat = self.controller.quat.copy()
        return {
            "joint_pos": q_policy,
    ...
    def reset_reference_to_state(self, state: Dict[str, np.ndarray], name: str = "anchor") -> None:
        joint_pos = np.asarray(state["joint_pos"], dtype=np.float32).reshape(1, -1)
        root_pos = np.asarray(state["root_pos"], dtype=np.float32).reshape(1, 3)
        root_quat = np.asarray(state["root_quat"], dtype=np.float32).reshape(1, 4)
        if joint_pos.shape[1] != self.n_joints:
            raise ValueError(f"Joint dim mismatch: got={joint_pos.shape[1]}, expected={self.n_joints}")

        qn = np.linalg.norm(root_quat, axis=1, keepdims=True)
        root_quat = root_quat / np.maximum(qn, 1e-6)
        self.ref_joint_pos = joint_pos.copy()
        self.ref_root_pos = root_pos.copy()
        self.ref_root_quat = root_quat.copy()
        self.ref_idx = 0
        self.ref_len = 1
